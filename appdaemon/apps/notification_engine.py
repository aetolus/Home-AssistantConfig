        ...
        while True:
            try:
                ...
                while localvars.Speech_InUse == 1:
                    ...
                    loop = loop + 1
                    if loop > 60:
                        self.log("Stuck in loop for 30 seconds, exiting.")
                        self.call_service("notify/telegram", message="Stuck in loop for 30 seconds, exiting.")
                        break
                ...
                self.call_service("tts/amazon_polly_say", entity_id=output_device, message='<speak><break time="2.5s" />' + voice_queue_text + '</speak>')
                # Rinse and repeat
