...
class GoogleCalendarEventDevice(CalendarEventDevice):
    ...
    def __init__(self, hass, calendar_service, calendar, data):
        ...
        _LOGGER.warning("USING CUSTOM COMPONENT FOR GOOGLE CALENDAR")
...
class GoogleCalendarData(object):
    ...
    def update(self):
        ...
        if 'dateTime' in items[0]['start']:
            self.event = items[0] if len(items) >= 1 else None
        elif 'dateTime' in items[1]['start']:
            self.event = items[1] if len(items) >= 1 else None
        else:
            None
        return True