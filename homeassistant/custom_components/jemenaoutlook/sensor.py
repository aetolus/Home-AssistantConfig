...
class JemenaOutlookClient(object):
    ...
    def _get_weekly_data(self, weeks_ago):
        ...
        try:
            ...
            _LOGGER.debug("exception data {}".format(e.errstring))
            raise JemenaOutlookError("Cannot get weekly data")
        try:
            json_output = raw_res.json()
    ...
    def _get_monthly_data(self, months_ago):
        ...
        try:
            ...
            _LOGGER.debug("exception data {}".format(e.errstring))
            raise JemenaOutlookError("Cannot get monthly data")
        try:
            json_output = raw_res.json()
    ...
    def fetch_data(self):
        ...
        try:
            ...
            raise JemenaOutlookError("Could not get daily data during fetch")

        # Get Daily Usage data
        try:
            self._data.update(self._get_weekly_data(0))
        except:
            raise JemenaOutlookError("Could not get weekly data during fetch")

        # Get Daily Usage data
        try:
            self._data.update(self._get_monthly_data(0))
        except:
            raise JemenaOutlookError("Could not get monthly data during fetch")
